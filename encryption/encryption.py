...
class Encryption:
    ...
    def write_key_folder(self, key_name):
        """
        ...
        """
        key_folder = os.path.abspath(os.path.join('..', 'key'))
    ...
    def write_key_rfid(self, key=None) :
        """
        key bytes a key generated by Fernet.generate_key(). If left to None the method will generate a new key
        If a key is passed, it will be written on a card
        """
        if key is None:
            key = Fernet.generate_key()
        str_key = key.decode()
    ...
    def load_key_folder(self, key_name):
        """
...
if __name__ == "__main__":
    x=Encryption()
    key_w = x.write_key_rfid()
    id, key_r = x.read_key_rfid()
